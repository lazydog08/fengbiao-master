...
以后小黑说“新增一个博主”或“新增风标”时，默认动作是把该博主近三年的公开视频封面和公开元数据回填到库内，按 `config/creators.json` 里的 `tags` 做分类，并缓存封面。新增博主默认只保留横版封面：配置里应开启 `landscape_only: true`；B 站默认用 `min_cover_aspect_ratio: 1.0` 保留宽大于高的横图，YouTube 可继续用 `1.6` 过滤 Shorts 类竖版内容。回填时会按实际缓存图片宽高剔除竖版或非横版封面，继续保留全局 `480x360` 坏样本拦截。
